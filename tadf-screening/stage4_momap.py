...
# Use the adjacent repository tooling by default. Set MOMAP_TOOLS_DIR when this
# adapter is deployed separately; do not assume a particular agent client.
TOOLS_DIR = os.path.expanduser(
    os.environ.get(
        'MOMAP_TOOLS_DIR',
        os.path.join(os.path.dirname(__file__), '..', 'momap', 'tools'),
    )
)
...
def run_single(
    ...
    try:
        ...
        try:
            ...
            error = result.stderr[-200:] or f'Invalid MOMAP JSON result file: {exc}'
            return {
                'mol_id': mol_id,
                'success': False,
                'error': error,
...
def generate_report(results, output_path, target_window=(450, 490), target_name='blue'):
    ...
    lines.extend([
        "",
        "---",
        "*Generated by the repository's Stage 4 MOMAP adapter; verify software and calculation provenance from source inputs.*"
    ])
...
def main():
    ...
    parser.add_argument(
        'input',
        nargs='?',
        help='CSV with columns: mol_id,s0_log,s1_log,t1_log,hso_cm1',
    )
    ...
    # Batch CSV mode
        ...
        with open(args.input) as f:
            ...
            for row in reader:
                ...
                raw_hso = row.get('hso_cm1', '').strip() if 'hso_cm1' in row else ''
                if raw_hso:
                    try:
                        hso_cm1 = float(raw_hso)
                    except ValueError:
                        print(f"  ⚠️  {mol_id}: invalid hso_cm1={raw_hso!r}, skipping")
                        results.append({
                            'mol_id': mol_id,
                            'success': False,
                            'error': f'invalid hso_cm1: {raw_hso!r}',
                        })
                        continue
